...
from tests.support.saltfactories_compat import DaemonFactory, SshdDaemonFactory
...
def ssh_pillar_tests_prep(request, salt_factories, salt_master, salt_minion):
    ...
    sshd_config = os.path.join(sshd_config_dir, "sshd_config")
    sshd_port = get_unused_localhost_port()
    url = "ssh://{username}@127.0.0.1:{port}/~/repo.git".format(
        username=request.cls.username, port=sshd_port
    )
    ...
    request.cls.ext_opts = {
        ...
        "privkey_nopass": os.path.join(home, request.cls.id_rsa_nopass),
        "pubkey_nopass": os.path.join(home, request.cls.id_rsa_nopass + ".pub"),
        "privkey_withpass": os.path.join(home, request.cls.id_rsa_withpass),
        "pubkey_withpass": os.path.join(home, request.cls.id_rsa_withpass + ".pub"),
        "passphrase": request.cls.passphrase,
    }
    ret = salt_call_cli.run(
        ...
        pillar={
            "git_pillar": {
                "git_ssh": request.cls.git_ssh,
                "id_rsa_nopass": request.cls.id_rsa_nopass,
                "id_rsa_withpass": request.cls.id_rsa_withpass,
                "sshd_bin": sshd_bin,
                "sshd_port": sshd_port,
                "sshd_config_dir": sshd_config_dir,
                "master_user": salt_master.config["user"],
                "user": request.cls.username,
            }
        },
        _timeout=240,
    )
    assert ret.exitcode == 0
    assert next(iter(ret.json.values()))["result"] is True

    sshd_proc = SshdDaemonFactory(
        cli_script_name=sshd_bin,
        config_dir=sshd_config_dir,
        listen_port=sshd_port,
        start_timeout=120,
...
def webserver_pillar_tests_prep(request, salt_factories, salt_master, salt_minion):
    ...
    nginx_conf = os.path.join(config_dir, "nginx.conf")
    uwsgi_conf = os.path.join(config_dir, "uwsgi.yml")
    git_dir = os.path.join(root_dir, "git")
    request.cls.repo_dir = repo_dir = os.path.join(git_dir, "repos")
    venv_dir = os.path.join(root_dir, "venv")
    uwsgi_bin = os.path.join(venv_dir, "bin", "uwsgi")
    request.cls.nginx_port = nginx_port = get_unused_localhost_port()
    uwsgi_port = get_unused_localhost_port()
    url = "http://127.0.0.1:{port}/repo.git".format(port=nginx_port)
    url_extra_repo = "http://127.0.0.1:{port}/extra_repo.git".format(port=nginx_port)
    ...
    for credential_param in ("user", "password"):
        if hasattr(request.cls, credential_param):
            ext_opts[credential_param] = getattr(request.cls, credential_param)
    request.cls.ext_opts = ext_opts

    auth_enabled = hasattr(request.cls, "username") and hasattr(request.cls, "password")
    pillar = {
        "git_pillar": {
    ...
    try:
        ret = salt_call_cli.run(
            "state.apply", mods="git_pillar.http", pillar=pillar, _timeout=240
        )
        assert ret.exitcode == 0
        assert next(iter(ret.json.values()))["result"] is True
