...
def _remove_stale_key(master, minion):
    key_path = os.path.join(master.config["pki_dir"], "minions", minion.id)
    log.debug("Minion %r KEY FILE: %s", minion.id, key_path)
    if os.path.exists(key_path):
        os.unlink(key_path)
    else:
        log.warning("The minion(id=%r) key was not found at %s", minion.id, key_path)
...
def salt_master_factory(
    ...
    master_id = random_string("pytest-master-")
...
def salt_minion_factory(salt_master_factory):
    ...
    factory = salt_master_factory.get_salt_minion_daemon(
        random_string("pytest-minion-"),
        config_defaults=config_defaults,
        config_overrides=config_overrides,
        extra_cli_arguments_after_first_start_failure=["--log-level=debug"],
    )
    factory.register_after_terminate_callback(
        _remove_stale_key, salt_master_factory, factory
    )
    return factory
...
def salt_sub_minion_factory(salt_master_factory):
    ...
    if virtualenv_binary:
        config_overrides["venv_bin"] = virtualenv_binary
    factory = salt_master_factory.get_salt_minion_daemon(
        random_string("pytest-sub-minion-"),
        config_defaults=config_defaults,
        config_overrides=config_overrides,
        extra_cli_arguments_after_first_start_failure=["--log-level=debug"],
    )
...
def salt_proxy_factory(salt_factories, salt_master_factory):
    proxy_minion_id = random_string("pytest-proxytest-")
